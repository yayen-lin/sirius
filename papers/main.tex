...
% TODO: Add a section opener, 1-2 sentences framing the whole section: exact join, fits on GPU, three reduction modes, expressed as Sirius operators

% TODO: we should briefly introduce super sirius first, and talk about how their relational operators were reused across these vector join operators
...
The join visits every pair of a probe batch and a corpus batch, runs that top-$k$ on the pair, and merges each probe row's results across all corpus batches into its final top-$k$.
As no step ever holds the full probe-by-corpus distance matrix, each pair is computed one tile at a time and reduced on the spot.

% TODO: do a code review and add any missing technical detail or design decision
\paragraph{Per-row} The per-row mode returns the $k$ nearest corpus rows for each probe row, and it is the base the other two modes build on.
Every (probe batch, corpus batch) pair goes through cuVS: \texttt{brute\_force::build} wraps the corpus batch as an index, and \texttt{brute\_force::search} returns, for each probe row in that pair, its $k$ nearest rows within that one corpus batch.
This search is where the GEMM and the internal tiling live, so a pair is scored without ever materializing its distance block in full.
Each probe row now hold one top-$k$ per corpus batch, and \texttt{knn\_merge\_parts} merges those per-batch lists into the row's final top-$k$ across the whole corpus. % TODO: mention cuVS API's internal tiling and GEMM here in this paragraph

\paragraph{Global} The global mode returns the $k$ nearest pairs across the whole join rather than $k$ per probe row.
It sits downstream of per-row: once the per-row reduction has each probe row's local top-$k$, the engine's existing \texttt{TOP\_N} and \texttt{MERGE\_TOP\_N} operators reduce those candidates into the single global top-$k$.
This mode reuses operators the engine already has, which is exactly the leverage of building the vector join out of ordinary relational operators.

\paragraph{Threshold} The threshold join mode returns every pair whose distance falls within a radius $\varepsilon$.
It is the one mode whose output size is not bounded. Top-$k$ yields a fixed number of results per probe row, but the number of pairs within $\varepsilon$ depends on the data and the threshold, so the output cannot be sized ahead of time.
Since cuVS's available APU presumes a bounded output, there is not an appropriate threshold search kernel that suits our use case.
...
The output is an edge list, one row per surviving (probe, corpus) pair carrying the two rwo ids and their distance.
And it is never sized in advance.
It starts empty and grows geometrically as each tile's survivors are appended in the same way a dynamic array does, so the total copy work stays linear in the number of surviving pairs and no counting pass is needed.
Because each tile is filtered the moment it is computed, the full distance matrix is never materialized; only one small tile and the surviving pairs sit in memory at any point.
Cosine runs through the same GEMM path with no need to normalize the input vectors first.
