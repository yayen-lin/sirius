...
As no step ever holds the full distance matrix, each pair is computed one tile at a time and reduced on the spot.
...
Since cuVS's available API presumes a bounded output, there is not an appropriate threshold search kernel that suits our use case.
...
The output is an edge list, one row per surviving (probe, corpus) pair carrying the two row ids and their distance.
There's no need to pre-allocate output buffer in advance: it starts empty and grows geometrically as each tile's survivors are appended in the same way a dynamic array does, so the total copy work stays linear in the number of surviving pairs and no counting pass is needed.
