...
\yxy{we don't need this subsection}
TBD
...
\yxy{I think S3.3 can be merged into S3.2. Just one subsection making a strong case for vector join as a new fundamental operator. }
...
In every case the work is inherently batched \yxy{maybe batched vector search itself is also a good example.}
...
Stonebraker and Pavlo argue that vector search is a feature, not an architecture, and that relational engines will simply absorb it~\cite{StonebrakerPavloAround24}\yxy{good reference. but probably better fit in S2.2.}.
...
\yxy{The point of this section is to argue a vector join is just another join. But according to S3.3, vector join is already integrated into real systems; so it is a bit awkward to argue for something that is already real. I suggest just expand S3.3 a little bit with the contents where, to argue briefly vector join should be a join operator, and use databricks/DuckDB as evidence.}
...
\subsection{Why GPU}\label{subsec:why-gpu}
...
\yxy{this subsection looks AI generated, which is ok for now. I think we don't need very long paragraphs here. Make the points more concise and clear. We can add some simple experiments to make the points very clear. Maybe two figures showing: (1) vector search in CPU vs. GPU (small gain) and (2) vector join in CPU vs. GPU (huge gain).
}

Relational OLAP is the heavier sibling of OLTP, and the intuition carries an expectation that the analytical vector workload should be heavier too\yxy{a better way to argue heavier is it's many-to-many instead of one-to-many}, but that is what makes it a better fit for the GPU\@.
...
\yxy{This is nice to have if we use it as a running example to explain the rest of the paper. Otherwise we don't need it and the new S3.1 will cover the gist of it.}
